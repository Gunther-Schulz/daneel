...
## da-9
grade: DONE
requirement: skill-craft cut-candidate predicate inherits a defect daneel measured and fixed on its own copy: a patch with no logged firing is called a cut candidate, but zero firings means two different things it cannot separate -- a patch that had opportunities and caught nothing (evidence against itself) reads identically to one minted last week (zero BY CONSTRUCTION). daneel surfaced this deliberately rather than patching skill-craft, and the surfacing lives ONLY in a file this arc is about to delete -- record: BACKLOG.md:198, the 2026-08-23 firing-log bullet, cross-repo paragraph
goal: general-maintenance
write-set: skill-craft: plugin/skills/skill-craft/SKILL.md (Durability classes) + its ITEMS.md booking
done-criterion: The finding is on skill-craft's OWN carrier, on its reader's path -- a daneel pointer alone is the decorative-reader failure, since no skill-craft context loads daneel's ITEMS.md. Delivered when: skill-craft's ITEMS.md carries an entry stating the defect with the daneel evidence pointer blob-pinned (so it resolves after daneel's BACKLOG.md is deleted), and this item closes with that entry's id. Whether skill-craft's rule text then CHANGES is that repo's decision and explicitly not this item's done-criterion -- booking the question is the deliverable, answering it is not.
evidence: MEASURED 2026-09-12. daneel fixed its own copy: dev-notes/OBSERVATIONS.md:18-38 carries the discriminator -- the reviewer's first question is not did it fire but has anything EXERCISED it yet, how many runs entered this rule's scope since minting; zero runs and zero firings carry no information, and such a rule is UNMEASURED rather than depreciating. skill-craft still carries the undefended predicate at plugin/skills/skill-craft/SKILL.md:368 and :478-479, and references/review-checklist.md:107. Its own journal records the OPPOSITE verdict at dev-notes/OBSERVATIONS.md:1240-1242 -- Durability cut-candidate gap CLOSED-ALREADY-SATISFIED because the rule text exists -- so a future reviewer there reads the gap as closed. Sweep over skill-craft for the defect formulation (zero runs / unmeasured / has anything exercised / run count / entered its scope) found only unrelated senses; positive control on the same instrument (cut candidate) returned 6 hits, so the zero is an instrument-proven absence, not an unread sweep.
blocked-by: NONE
blocker-moot: whether skill-craft Durability-classes takes the run-count discriminator, which skill-craft own Reflexivity rule reserves to the operator
closed-reason: 2026-09-12 Delivered to the consumer's own carrier, verified at ORIGIN rather than at my tree: skill-craft ITEMS.md now carries sc-6, commit ddb6521 (skill-craft repo, so it is named here rather than passed as --ref, which verifies against this repo only), confirmed present in origin/main:ITEMS.md. The daneel pointer inside it is blob-pinned at f99ff84f15b8c8bd2b7b89793b3dddd8252d0376, so it keeps resolving after this repo's BACKLOG.md is deleted. The write boundary was granted narrowly by the judgment desk for exactly one item add -- no SKILL.md, no references/* -- and the push was released on the target's OWN publication posture (skill-craft declares public: true and its ITEMS.md is an already-published tracked file), not on an analogy to another repo. Held open deliberately until the push landed: the sender's clean tree reading as delivery is the residue failure this item existed to prevent.
