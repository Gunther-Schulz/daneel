...
source-blob: f99ff84f15b8c8bd2b7b89793b3dddd8252d0376  (BACKLOG.md)
...
| quantity | count |
|---|---|
| top-level bullets in `BACKLOG.md` | 12 |
| of those, ENTRIES (bold, or led by a grade-shaped word) | 9 |
| of those, non-entry prose bullets (not migrated) | 3 |
| of those, bullets in a section §4 row 1 CUTS | 0 |
| items written to `ITEMS.md` | 6 |
| of those, CLOSURES routed to `ITEMS-DONE.md` instead | 3 |
| entries reported UNCLASSIFIED or AMBIGUOUS (not written) | 0 |
| archive bodies in `ITEMS-DONE.md` (verbatim) | 3 |
| entries routed to the ledger | 0 |
| RESIDUE items booked (`tend`, from no source entry) | 1 |

**Identity:** 9 entries read = 6 written + 3 closed + 0 unclassified — HOLDS. THREE columns, not two: a closure read out of the source carrier is neither written as an item nor unclassified, and folding it into either would make one of those numbers say something it does not.

**Bullet identity:** 12 top-level bullets = 9 entries + 3 prose + 0 cut — HOLDS. This is the identity that makes 'not migrated' visible: every bullet in the source is in exactly one of the three columns, so a bullet the migration simply did not see would show up as a gap in the sum rather than as nothing at all.

**Conservation (§3.1), computed on the produced files:** items 7 (of which 1 residue) + done 3 = 10; baseline 10 + added 0 − compacted 0 = 10. HOLDS. THE RESIDUE IS IN THIS IDENTITY AND NOT IN THE ONE ABOVE, and the split is the point: a residue item is a BODY in the carrier, so conservation must see it, and it comes from no SOURCE ENTRY, so the reconciliation identity must not — folding it in there would answer COULD NOT VERIFY on every migration after this one.
...
|---|---|---|
| `PARKED` | NEW | 4 |
| `(ungraded)` | CLOSED → the done home | 3 |
| `READY` | READY | 2 |
...
- `BACKLOG.md:179` — section: Open
- `BACKLOG.md:198` — section: Open
- `BACKLOG.md:239` — section: Open

## Closures routed to the done home

3 entry/ies. Each body is in `ITEMS-DONE.md` VERBATIM, under the archive heading, preceded by a comment naming its source line range and the rule that routed it.

| line | source grade word | section | rule |
|---|---|---|---|
| `BACKLOG.md:299-325` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:326-353` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
| `BACKLOG.md:354-393` | `(ungraded)` | Done | §4 row 1 + lc-18: ungraded, under the carrier's own closure heading — the heading is the closure statement, an |
...
**1 tracked file(s)** still name `BACKLOG.md` — one item, its `write-set` and `evidence` naming every one of them:
