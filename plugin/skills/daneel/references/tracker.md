...
The basis field carries what the evidence ESTABLISHES and not
only which artifact it is — `basis: F29, two byte-identical specs
differing; establishes that an unconfigured variable exists` — so
the basis rule's entailment edge (`foundations.md`) is readable
off the ledger line rather than reconstructed from it. A verdict
whose summary claims more than its basis establishes is malformed
whatever its status.
...
The impl plan is produced at [READY] presentation so the
operator sees the unit list at the decision moment
(`references/closed-artifact.md` State summary): dispatch units
derived from the locked fix design, dependency-ordered,
